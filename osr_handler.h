...
public:
	...
	// Called when shared_texture_enabled = true (replaces OnPaint)
	...
	// Kept as no-op fallback (should not be called with shared_texture_enabled)
...
private:
	...
	// Creates (or recreates) the persistent shared D3D11 texture and exports its NT handle.
	// Called from OnAcceleratedPaint when size changes.
	...
	// Popup (still CPU path - no accelerated popup support)
	CefRect              m_popupRect;
	std::vector<uint8_t> m_popupBuffer;
	std::atomic<bool>    m_popupVisible{ false };

	// D3D11 shared texture (persistent copy target)
	ComPtr<ID3D11Texture2D> m_sharedTexture;   // DXGI shared, UE opens this
	HANDLE                  m_sharedNTHandle = nullptr; // exported NT handle
