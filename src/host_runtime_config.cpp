...
HostRuntimeConfig HostRuntimeConfig::FromArgs(int argc, char* argv[])
{
	...
	for (int i = 1; i < argc; ++i)
	{
		...
		{
			if (!value.empty()) { dst = value; return true; }
		...
		{
			std::string v;
			if (takeNext(v) && !v.empty()) cfg.StartupUrl = v;
			continue;
		}
		if (key == "--width")
		{
			std::string v;
			uint32_t n = 0;
			if (takeNext(v) && ParseUInt(v, n)) cfg.Width = std::max<uint32_t>(1, n);
			continue;
		}
		if (key == "--height")
		{
			std::string v;
			uint32_t n = 0;
			if (takeNext(v) && ParseUInt(v, n)) cfg.Height = std::max<uint32_t>(1, n);
			continue;
		}
		if (key == "--fps")
		{
			std::string v;
			int32_t n = 0;
			if (takeNext(v) && ParseInt(v, n)) cfg.FrameRate = std::clamp(n, 1, 240);
			continue;
		}
		if (key == "--size")
		{
			std::string v;
			if (!takeNext(v)) continue;
